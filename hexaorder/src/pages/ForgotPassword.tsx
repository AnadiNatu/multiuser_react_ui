...
export default function ForgotPassword() {
  ...
  const handleRequest = async (e: React.FormEvent) => {
    ...
    setError(''); setInfo(''); setLoading(true);
    try {
      const res = await apiService.postQuery<any>(
        API_ENDPOINTS.PASSWORD_FORGOT,
        { email: identifier, method: method.toUpperCase() }
      );
      setInfo(res.message || 'OTP sent successfully');
      setStep('verify');
    } catch (err: any) {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await apiService.postQuery<any>(
        ...
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl border border-white/20 shadow-2xl p-8">
          ...
          {step === 'request' ? (
            <form onSubmit={handleRequest} className="space-y-4">
              ...
              <Input label="Email Address" type="email" value={identifier}
            ...
            <form onSubmit={handleReset} className="space-y-4">
              <Input label="OTP Code" type="text" value={otp}
                onChange={(e) => setOtp(e.target.value)} placeholder="Enter 6-digit OTP" required />
              <Input label="New Password" type="password" value={newPassword}
                onChange={(e) => setNewPassword(e.target.value)}
                leftIcon={<Lock className="w-4 h-4" />} placeholder="Min 8 characters" required />
              <Input label="Confirm Password" type="password" value={confirmPassword}
                onChange={(e) => setConfirmPassword(e.target.value)}
                leftIcon={<Lock className="w-4 h-4" />} placeholder="Repeat password" required />
              <div className="flex gap-3">
                <Button type="button" variant="outline" onClick={() => setStep('request')} className="flex-1">Back</Button>
                <Button type="submit" isLoading={loading} className="flex-1">Reset Password</Button>
              </div>
